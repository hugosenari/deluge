...
class CreateTorrentDialog(object):
    ...
    def show(self):
        ...
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'create_torrent_dialog.ui'),
        ) or '')
        # The remote path dialog
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'create_torrent_dialog.remote_path.ui'),
        ))
        # The remote save dialog
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'create_torrent_dialog.remote_save.ui'),
        ))
        # The progress dialog
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'create_torrent_dialog.progress.ui'),
        ))

        self.config = ConfigManager('giui.conf')
    ...
    def _on_create_torrent_progress(self, value, num_pieces):
        ...
        if percent >= 0 and percent <= 1.0:
            ...
            GLib.idle_add(update_pbar_with_gobject, percent)
