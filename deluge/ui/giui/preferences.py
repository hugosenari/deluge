...
class Preferences(component.Component):
    def __init__(self):
        ...
        self.builder.add_from_file(deluge.common.resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'preferences_dialog.ui'),
        ))
        ...
        self.giui_config = ConfigManager('giui.conf')
        ...
        from .gigtk import DEFAULT_PREFS
        self.COLOR_DEFAULTS = {}
    ...
    def load_languages(self):
        ...
        for i, l in enumerate(langs):
            ...
            if self.giui_config['language'] == lang_code:
                index = i

        if self.giui_config['language'] is None:
    ...
    def _show(self):
        ...
        # Downloads tab #
        self.builder.get_object('chk_show_dialog').set_active(self.giui_config['interactive_add'])
        self.builder.get_object('chk_focus_dialog').set_active(self.giui_config['focus_add_dialog'])

        # Interface tab #
        self.builder.get_object('chk_use_tray').set_active(self.giui_config['enable_system_tray'])
        self.builder.get_object('chk_min_on_close').set_active(self.giui_config['close_to_tray'])
        self.builder.get_object('chk_start_in_tray').set_active(self.giui_config['start_in_tray'])
        self.builder.get_object('radio_appind').set_active(self.giui_config['enable_appindicator'])
        self.builder.get_object('chk_lock_tray').set_active(self.giui_config['lock_tray'])
        self.builder.get_object('radio_standalone').set_active(self.giui_config['standalone'])
        self.builder.get_object('radio_thinclient').set_active(not self.giui_config['standalone'])
        self.builder.get_object('chk_show_rate_in_title').set_active(self.giui_config['show_rate_in_title'])
        self.builder.get_object('chk_focus_main_window_on_add').set_active(
            self.giui_config['focus_main_window_on_add'],
        )
        self.builder.get_object('piecesbar_toggle').set_active(self.giui_config['show_piecesbar'])
        ...
        # Other tab #
        self.builder.get_object('chk_show_new_releases').set_active(self.giui_config['show_new_releases'])
    ...
    def set_config(self, hide=False):
        ...
        # Downloads tab #
        new_giui_config['interactive_add'] = self.builder.get_object('chk_show_dialog').get_active()
        new_giui_config['focus_add_dialog'] = self.builder.get_object('chk_focus_dialog').get_active()

        for state in ('missing', 'waiting', 'downloading', 'completed'):
            color = self.builder.get_object('%s_color' % state).get_color()
            new_giui_config['pieces_color_%s' % state] = [
        ...
        # Interface tab #
        new_giui_config['enable_system_tray'] = self.builder.get_object('chk_use_tray').get_active()
        new_giui_config['close_to_tray'] = self.builder.get_object('chk_min_on_close').get_active()
        new_giui_config['start_in_tray'] = self.builder.get_object('chk_start_in_tray').get_active()
        new_giui_config['enable_appindicator'] = self.builder.get_object('radio_appind').get_active()
        new_giui_config['lock_tray'] = self.builder.get_object('chk_lock_tray').get_active()
        passhex = sha(self.builder.get_object('txt_tray_password').get_text().encode('UTF-8')).hexdigest()
        if passhex != 'c07eb5a8c0dc7bb81c217b67f11c3b7a5e95ffd7':
            new_giui_config['tray_password'] = passhex

        was_standalone = self.giui_config['standalone']
        new_giui_standalone = self.builder.get_object('radio_standalone').get_active()
        new_giui_config['standalone'] = new_giui_standalone

        new_giui_config['show_rate_in_title'] = self.builder.get_object(
            'chk_show_rate_in_title',
        ).get_active()
        new_giui_config['focus_main_window_on_add'] = self.builder.get_object(
            'chk_focus_main_window_on_add',
        ).get_active()

        # Other tab #
        new_giui_config['show_new_releases'] = self.builder.get_object(
        ...
        if self.language_checkbox.get_active():
            ...
            if active == -1:
                ...
                new_giui_config['language'] = model.get(model.get_iter(active), 0)[0]

        if new_giui_config['language'] != self.giui_config['language']:
            dialog = InformationDialog(
                _('Attention'),
        ...
        # GtkUI
        for key in new_giui_config:
            # The values do not match so this needs to be updated
            if self.giui_config[key] != new_giui_config[key]:
                self.giui_config[key] = new_giui_config[key]
        ...
        if was_standalone != new_giui_standalone:
            def on_response(response):
                if response == Gtk.ResponseType.YES:
                    ...
                    self.giui_config['standalone'] = not new_giui_standalone
                    self.builder.get_object('radio_standalone').set_active(
                        self.giui_config['standalone'],
                    )
                    self.builder.get_object('radio_thinclient').set_active(
                        not self.giui_config['standalone'],
                    )
    ...
    def load_pref_dialog_state(self):
        w = self.giui_config['pref_dialog_width']
        h = self.giui_config['pref_dialog_height']
        if w is not None and h is not None:
            self.pref_dialog.resize(w, h)

    def on_pref_dialog_configure_event(self, widget, event):
        self.giui_config['pref_dialog_width'] = event.width
        self.giui_config['pref_dialog_height'] = event.height
    ...
    def on_test_port_clicked(self, data):
        ...
        #      It requires Gtk.MAJOR_VERSION > 3
    ...
    def on_piecesbar_toggle_toggled(self, widget):
        self.giui_config['show_piecesbar'] = widget.get_active()
    ...
    def __set_color(self, state, from_config=False):
        if from_config:
            color = Gdk.Color(*self.giui_config['pieces_color_%s' % state])
            ...
            self.giui_config['pieces_color_%s' % state] = [color.red, color.green, color.blue]
            self.giui_config.save()
            self.giui_config.apply_set_functions('pieces_colors')
