...
class GiUI(object):
    def __init__(self, args):
        ...
        # Make sure giui.conf has at least the defaults set
        self.config = ConfigManager('giui.conf', DEFAULT_PREFS)

        # Make sure the giui state folder has been created
        if not os.path.exists(os.path.join(get_config_dir(), 'giui_state')):
            os.makedirs(os.path.join(get_config_dir(), 'giui_state'))
        ...
        # Initialize various components of the giui
        ...
        reactor.addSystemEventTrigger('before', 'giui_close', self.close)

        def giui_sigint_handler(num, frame):
            log.debug('SIGINT signal caught, firing event: giui_close')
            reactor.callLater(0, reactor.fireSystemEvent, 'giui_close')

        signal.signal(signal.SIGINT, giui_sigint_handler)

    def start(self):
        log.debug('GIUI starting up...')
    ...
    def _on_reactor_start(self):
        ...
        try:
            ...
            component.start()
            log.debug('SPAAAAAAAAAAARRRRRRRRTTTAAAAAAAAAAA!!!')
            return
    ...
    def _start_thinclient(self):
        """Start the giui in thinclient mode"""
