...
class ConnectionManager(component.Component):
    ...
    def show(self):
        ...
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'connection_manager.ui'),
        ))
        ...
        # Set widgets to values from giui config.
    ...
    def _load_widget_config(self):
        ...
        self.builder.get_object('chk_autoconnect').set_active(
            self.giui_config['autoconnect'],
        )
        self.builder.get_object('chk_autostart').set_active(
            self.giui_config['autostart_localhost'],
        )
        self.builder.get_object('chk_donotshow').set_active(
            not self.giui_config['show_connection_manager_on_start'],
        )
    ...
    def _on_connect(self, daemon_info, host_id):
        ...
        if self.giui_config['autoconnect']:
            self.giui_config['autoconnect_host_id'] = host_id
        if self.running:
    ...
    def _on_connect_fail(self, reason, host_id, try_counter):
        ...
        if not self.giui_config['autostart_localhost']:
            msg += '\n' + _(
    ...
    def _run_addhost_dialog(self, edit_host_info=None):
        ...
        """
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'connection_manager.addhost.ui'),
        ))
    ...
    def on_chk_toggled(self, widget):
        self.giui_config['autoconnect'] = self.builder.get_object('chk_autoconnect').get_active()
        self.giui_config['autostart_localhost'] = self.builder.get_object('chk_autostart').get_active()
        self.giui_config['show_connection_manager_on_start'] = not self.builder.get_object(
            'chk_donotshow',
        ).get_active()
