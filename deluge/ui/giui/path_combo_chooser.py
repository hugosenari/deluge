...
class PathChooserPopup(object):
    ...
    def get_position(self):
        ...
        height_extra = 8
        buttonbox_width = 0
        height = self.popup_window.get_size_request()[1]
        width = self.popup_window.get_size_request()[0]

        if self.popup_buttonbox:
            buttonbox_height = max(self.popup_buttonbox.get_size_request()[1], self.popup_buttonbox.get_allocation().height)
            buttonbox_width = max(self.popup_buttonbox.get_size_request()[0], self.popup_buttonbox.get_allocation().width)
            treeview_width = self.treeview.get_size_request()[0]
            # After removing an element from the tree store, self.treeview.get_size_request()[0]
        ...
        # 10 is extra spacing
        content_width = self.treeview.get_size_request()[0] + buttonbox_width + 10
        ...
        if len(self.tree_store) > 0 and self.max_visible_rows > 0:
            ...
            self.row_height = self.treeview.get_size_request()[1] // len(self.tree_store)
...
#################
    ...
    def on_buttonbox_key_press_event(self, widget, event):
        ...
        keyval = event.keyval
        state = event.get_state() & Gtk.accelerator_get_default_mod_mask()
        if keyval == Gdk.KEY_Escape or (key_is_up(keyval) and state == Gdk.ModifierType.MOD1_MASK):
            self.popdown()
            return True
        return False
...
class PathChooserComboBox(Gtk.HBox, StoredValuesPopup, GObject.GObject):
    ...
    def __init__(self, max_visible_rows=20, auto_complete=True, use_completer_popup=True):
        ...
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'path_combo_chooser.ui'),
        ))
