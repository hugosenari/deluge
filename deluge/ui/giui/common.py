...
#
"""Common functions for various parts of giui to use."""
...
def create_blank_pixbuf(size=16):
    pix = GdkPixbuf.Pixbuf(GdkPixbuf.Colorspace.RGB, True, 8, size, size)
    pix.fill(0x0)
    return pix
...
def get_pixbuf(filename):
    try:
        return GdkPixbuf.Pixbuf.new_from_file(get_pixmap(filename))
...
def get_pixbuf_at_size(filename, size):
    try:
        return GdkPixbuf.Pixbuf.new_from_file_at_size(get_pixmap(filename), size, size)
    except GLib.GError as ex:
...
def save_pickled_state_file(filename, state):
    ...
    filepath = os.path.join(get_config_dir(), 'giui_state', filename)
...
def get_clipboard_text():
    text = Gtk.Clipboard.get(Gdk.SELECTION_PRIMARY).wait_for_text() or \
