...
class EditTrackersDialog(object):
    def __init__(self, torrent_id, parent=None):
        ...
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'edit_trackers.ui'),
        ))
        # add tracker dialog
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'edit_trackers.add.ui'),
        ))
        # edit tracker dialog
        self.builder.add_from_file(resource_filename(
            'deluge.ui.giui', os.path.join('glade', 'edit_trackers.edit.ui'),
        ))
    ...
    def load_edit_trackers_dialog_state(self):
        w = self.giui_config['edit_trackers_dialog_width']
        h = self.giui_config['edit_trackers_dialog_height']
        if w is not None and h is not None:
            self.dialog.resize(w, h)

    def on_edit_trackers_dialog_configure_event(self, widget, event):
        self.giui_config['edit_trackers_dialog_width'] = event.width
        self.giui_config['edit_trackers_dialog_height'] = event.height
