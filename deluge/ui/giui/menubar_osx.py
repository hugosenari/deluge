...
def accel_meta(item, group, key):
    accel_swap(item, group, key, Gdk.ModifierType.CONTROL_MASK, key, Gdk.ModifierType.META_MASK)


def menubar_osx(giui, osxapp):
    main_builder = giui.mainwindow.get_builder()
    menubar = main_builder.get_object('menubar')
    group = Gtk.accel_groups_from_object(giui.mainwindow.window)[0]

    config = ConfigManager('giui.conf')
    ...
    accel_swap(quit_all_item, group, 'q', Gdk.ModifierType.SHIFT_MASK | Gdk.ModifierType.CONTROL_MASK, 'q', Gdk.ModifierType.SHIFT_MASK | Gdk.ModifierType.META_MASK)
    ...
    accel_swap(pref_item, group, 'p', Gdk.ModifierType.CONTROL_MASK, ',', Gdk.ModifierType.META_MASK)
