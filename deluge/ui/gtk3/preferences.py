...
class Preferences(component.Component):
    ...
    def _show(self):
        ...
        # Downloads tab #
        self.builder.get_object('chk_show_dialog').set_active(self.config['interactive_add'])
        self.builder.get_object('chk_focus_dialog').set_active(self.config['focus_add_dialog'])

        # Interface tab #
        self.builder.get_object('chk_use_tray').set_active(self.config['enable_system_tray'])
        self.builder.get_object('chk_min_on_close').set_active(self.config['close_to_tray'])
        self.builder.get_object('chk_start_in_tray').set_active(self.config['start_in_tray'])
        self.builder.get_object('radio_appind').set_active(self.config['enable_appindicator'])
        self.builder.get_object('chk_lock_tray').set_active(self.config['lock_tray'])
        self.builder.get_object('radio_standalone').set_active(self.config['standalone'])
        self.builder.get_object('radio_thinclient').set_active(not self.config['standalone'])
        self.builder.get_object('chk_show_rate_in_title').set_active(self.config['show_rate_in_title'])
        self.builder.get_object('chk_focus_main_window_on_add').set_active(
            self.config['focus_main_window_on_add'],
        )
        self.builder.get_object('piecesbar_toggle').set_active(self.config['show_piecesbar'])
        ...
        # Other tab #
        self.builder.get_object('chk_show_new_releases').set_active(self.config['show_new_releases'])
    ...
    def set_config(self, hide=False):
        ...
        # Downloads tab #
        new_config['interactive_add'] = self.builder.get_object('chk_show_dialog').get_active()
        new_config['focus_add_dialog'] = self.builder.get_object('chk_focus_dialog').get_active()

        for state in ('missing', 'waiting', 'downloading', 'completed'):
            color = self.builder.get_object('%s_color' % state).get_color()
            new_config['pieces_color_%s' % state] = [
        ...
        # Interface tab #
        new_config['enable_system_tray'] = self.builder.get_object('chk_use_tray').get_active()
        new_config['close_to_tray'] = self.builder.get_object('chk_min_on_close').get_active()
        new_config['start_in_tray'] = self.builder.get_object('chk_start_in_tray').get_active()
        new_config['enable_appindicator'] = self.builder.get_object('radio_appind').get_active()
        new_config['lock_tray'] = self.builder.get_object('chk_lock_tray').get_active()
        passhex = sha(self.builder.get_object('txt_tray_password').get_text().encode('UTF-8')).hexdigest()
        if passhex != 'c07eb5a8c0dc7bb81c217b67f11c3b7a5e95ffd7':
            new_config['tray_password'] = passhex

        was_standalone = self.config['standalone']
        new_standalone = self.builder.get_object('radio_standalone').get_active()
        new_config['standalone'] = new_standalone

        new_config['show_rate_in_title'] = self.builder.get_object(
            'chk_show_rate_in_title',
        ).get_active()
        new_config['focus_main_window_on_add'] = self.builder.get_object(
            'chk_focus_main_window_on_add',
        ).get_active()

        # Other tab #
        new_config['show_new_releases'] = self.builder.get_object(
        ...
        if self.language_checkbox.get_active():
            ...
            if active == -1:
                ...
                new_config['language'] = model.get(model.get_iter(active), 0)[0]
        ...
        if was_standalone != new_standalone:
            def on_response(response):
                if response == Gtk.ResponseType.YES:
                    ...
                    self.config['standalone'] = not new_standalone
    ...
    def load_pref_dialog_state(self):
        w = self.config['pref_dialog_width']
        h = self.config['pref_dialog_height']
        if w is not None and h is not None:
            self.pref_dialog.resize(w, h)

    def on_pref_dialog_configure_event(self, widget, event):
        self.config['pref_dialog_width'] = event.width
        self.config['pref_dialog_height'] = event.height
    ...
    def on_piecesbar_toggle_toggled(self, widget):
        self.config['show_piecesbar'] = widget.get_active()
    ...
    def __set_color(self, state, from_config=False):
        if from_config:
            color = Color(*self.config['pieces_color_%s' % state])
            ...
            self.config['pieces_color_%s' % state] = [color.red, color.green, color.blue]
            self.config.save()
            self.config.apply_set_functions('pieces_colors')
